...
int cut_invariant_mass_signal_onlytracks() {
    ...
    for (const auto& filename : filenames) {
        ...
        for (Long64_t i = 0; i < tree->GetEntries(); ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                // Simplificar el criterio de b-tagging: más de 2 tracks
                if (num_tracks > 2) {
                    TLorentzVector jet;
    ...
    TFile* output_file = TFile::Open("cut_invariant_mass_jets_histogram_signal_400_only_tracks.root", "RECREATE");
    if (output_file) {
