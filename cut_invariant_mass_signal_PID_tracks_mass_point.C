...
int cut_invariant_mass_signal_PID_tracks_mass_point() {
    ...
    for (const auto& filename : filenames) {
        ...
        for (Long64_t i = 0; i < tree->GetEntries(); ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                unsigned int tracks_begin = jet_tracks_begin[j];
                unsigned int tracks_end = jet_tracks_end[j];
                ...
                // Criterio de b-tagging: tracks_begin y tracks_end en [0, 1], masa mayor a 5.0, goodnessOfPID menor a 0.02 y desplazamiento significativo
                if (tracks_begin >= 0 && tracks_begin <= 1 && tracks_end >= 0 && tracks_end <= 1 && mass > 5.0 && goodnessOfPID < 0.02 && displacement > 0.01) {
                    TLorentzVector jet;
    ...
    TFile* output_file = TFile::Open("cut_invariant_mass_jets_histogram_signal_PID_tracks_mass_point_400.root", "RECREATE");
    if (output_file) {
